...
def cloudplus_login():
    ...
    while True:
        try:
            ...
            print u'获取access_token成功：' + access_token
            return access_token
        except Exception as e:
            print e
            print u'获取access_token失败，等待' + str(RELOGIN_INTERVAL) + u's重新获取...'
    ...
    while True:
        try:
            ...
            if len(response.text) is 34:
                print u'预定成功！地点：' + book_info['room']['name'] + '\n'
                break
            else:
                try:
                    records = json.loads(response.text)
                    print u'预定失败！msg：' + records['msg'] + '\n'
                    break
                except Exception as e:
                    print e
                    print u'预定失败！\n'
...
def get_room_schedule(access_token):
    url = 'https://ecm.inspuronline.com/inspur_esg/schedule-ext/api/schedule/v6.0/meeting/GetRoomUse?endTime=1560096000000&roomId=61&startTime=1559232000000'
    headers = {
        'Host': 'ecm.inspuronline.com',
        'Cookie': 'X-ECC-Current-Enterprise=10000; x-ecp-token=bearer%20' + access_token,
        'X-Device-ID': '64d72692c8a8f7b3643ef84fe1680ddf706df2e2',
...
def get_start_end_timestamp(start_time, end_time):
    ...
    start_datetime = today_0 + datetime.timedelta(days=2, hours=start_time)
    end_datetime = today_0 + datetime.timedelta(days=2, hours=end_time)
    print u'会议开始时间：' + str(start_datetime) + u' 结束时间：' + str(end_datetime)
...
if __name__ == '__main__':
    ...
    # print(timestamp2str(1559350800))
